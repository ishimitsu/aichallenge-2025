...
# Single Lap Evaluation Script
# Modified version of run_evaluation.bash for single lap timing
...
cleanup() {
    ...
    echo "Single lap evaluation cleanup complete."
    # Stop ros2 daemon
    ros2 daemon stop
    exit 0
}
...
monitor_single_lap() {
    ...
    while true; do
        ...
        # Timeout check
        if [ $elapsed_time -ge $max_lap_time ]; then
            echo "Single lap timeout reached (${max_lap_time}s). Stopping evaluation."
            return 1
        fi
        
        # Log progress every 10 seconds
        if [ $((elapsed_time % 10)) -eq 0 ]; then
            echo "Single lap monitoring: ${elapsed_time}s elapsed"
        fi
...
OUTPUT_DIRECTORY=$(date +%Y%m%d-%H%M%S)-single-lap
cd /output || exit
mkdir "$OUTPUT_DIRECTORY"
ln -nfs "$OUTPUT_DIRECTORY" latest-single-lap
...
# Start AWSIM with endless mode (we'll stop it manually after 1 lap)
echo "Start AWSIM (endless mode for single lap evaluation)"
...
# Monitor for single lap completion instead of waiting for AWSIM to finish
echo "Starting single lap evaluation monitoring..."
monitor_single_lap

# Single lap completed or timed out, initiate cleanup
echo "Single lap evaluation finished, initiating cleanup..."
...
echo "Single lap evaluation complete"
